...
const TableCompany = ({ companies }) => {
  ...
  const deleteCompany = async (id) => {
    try {
      ...
      setMessage(false);
      await axios
        .delete(`https://datawerehouse.herokuapp.com/v1/api/companies/${id}`, {
          headers: { Authorization: JWT },
        })
        .then((res) => {
          setIdFullCompany([]);
      ...
      if (name.indexOf(valueInput) !== -1) {
        let filter = rows.filter(
          (row) =>
            ...
            company.name_company.toLowerCase()
        );
      ...
      if (country.indexOf(valueInput) !== -1) {
        const filterCountry = rows.filter(
          (row) =>
            ...
            company.Country.name_country.toLowerCase()
        );
  ...
  return (
    <>
      ...
      {message ? (
        <CustomizedSnackbars message="Compañía eliminada con éxito" />
      ) : (
        ""
      )}
      <div>
