...
const Formcountry = ({ regions, countries }) => {
  ...
  const registerCountry = async (e) => {
    ...
    try {
      await axios
        .post("https://datawerehouse.herokuapp.com/v1/api/countries", data, {
          headers: {
            Authorization: JWT,
  ...
  const deleteCountry = async (e) => {
    ...
    try {
      id.forEach((element, index) => {
        axios
          .delete(
            `https://datawerehouse.herokuapp.com/v1/api/countries/${element}`,
            {
  ...
  return (
    <main className="container__form__region">
      <div className="container__title__region">
        <TitleWelcomeForm title="¡Muy bien! Ahora debes agregar los países según cada región, recuerda que también puedes agregar varios países" />
      </div>
      <div className="container__form__flex">
        <div className="form__input">
          <h6 className="region__title">País</h6>
          <form
            onSubmit={registerCountry.bind()}
          ...
          >
            <FormControl className="input" variant="outlined" margin="dense">
              ...
              <Select name="regionSelected" native label="Region">
                <option aria-label="None" value="" />
                {regions
                  ? regions.map((r) => (
                      <option
                        value={`${r.id_region}`}
                      >{`${r.name_region}`}</option>
                    ))
